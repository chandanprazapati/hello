...
import { createBusinessStatus } from "../../../services/apiServices/revenue/businessStatus/businessStatusService";
import AddButton from "../../reusableDesign/AddButton";

const CreateBusinessStatus = ({ clickedIdData }) => {
  ...
  const onSubmit = useCallback(
    async (data) => {
      try {
        ...
        const response = await createBusinessStatus(data);
        if (response.status === true) {
          toast.success(response.message, {
            icon: "🚀",
            autoClose: 1000,
          });
          router.push("/revenue/businessStatus");
  ...
  return (
    <>
      <CommonHeaderDesign title={"व्यापार-स्थिति राख्नुहोस"} />
