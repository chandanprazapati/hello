...
export default function TaxSubCategory() {
  const router = useRouter();
  const [apiData, setApiData] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let taxSubCategoryApiData = () => {
    ...
    taxSubCategoryApiData();
  }, [setApiData]);

  const handleEdit = (id) => {
    router.push(`/revenue/taxSubCategory/createTaxSubCategory/${id}`);
  };
      ...
      <TableBody>
        {apiData.map((row, index) => (
          <TableRow key={index} className="hover:bg-[#a0cae7fd]">
            ...
            <TableCell>{row.taxCategoryName}</TableCell>
            <TableCell
  ...
  return (
    <ListViewPageDesign>
      <ListHeader title="कर उपवर्ग सूची" />
      <ListButton url={`/revenue/taxSubCategory/createTaxSubCategory`} />
...
const tableHeadData = [
  { id: "id", name: "सि.नं." },
  { id: "name", name: "नाम" },
  { id: "taxCategoryName", name: "कर वर्ग नाम" },
