...
export default function VehicleStatus() {
  ...
  useEffect(() => {
    let vehicleStatusApiData = () => {
      vehicleStatus().then((response) => {
        try {
    ...
    vehicleStatusApiData();
  }, [setApiData]);

  const handleEdit = (id) => {
    router.push(`/revenue/vehicleStatus/createVehicleStatus/${id}`);
  };
  const locale = (
  ...
  return (
    <ListViewPageDesign>
      <ListHeader title="वाहन-स्थिति सूची" />
      <ListButton url={`/revenue/vehicleStatus/createVehicleStatus`} />
