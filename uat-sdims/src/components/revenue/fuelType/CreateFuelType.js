...
import { createFuelType } from "../../../services/apiServices/revenue/fuelType/fuelTypeService";
import AddButton from "../../reusableDesign/AddButton";

export default function CreateFuelType({ clickedIdData }) {
  const {
    register,
    handleSubmit,
  ...
  const onSubmit = useCallback(
    async (data) => {
      try {
        ...
        const response = await createFuelType(data);
        if (response.status === true) {
          toast.success(response.message, {
            icon: "🚀",
            autoClose: 1000,
          });
          router.push("/revenue/fuel");
  ...
  return (
    <>
      <CommonHeaderDesign title={"इन्धन प्रकार राख्नुहोस"} />
