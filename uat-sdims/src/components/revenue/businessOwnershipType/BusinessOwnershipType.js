...
export default function BusinessOwnershipType() {
  const router = useRouter();
  const [apiData, setApiData] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let businessOwnershipTypeApiData = () => {
      businessOwnershipType().then((response) => {
        try {
    ...
    businessOwnershipTypeApiData();
  }, [setApiData]);

  const handleEdit = (id) => {
    router.push(`/revenue/businessOwner/createBusinessOwner/${id}`);
  };
  ...
  return (
    <ListViewPageDesign>
      <ListHeader title="व्यापार-स्वामित्व प्रकार सूची" />
      <ListButton url={`/revenue/businessOwner/createBusinessOwner`} />
