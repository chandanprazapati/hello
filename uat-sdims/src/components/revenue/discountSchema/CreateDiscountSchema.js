...
import { discountSchemaValidationResolver } from "../../../utils/validateField";
import { taxModule } from "../../../services/apiServices/revenue/taxModule/taxModuleService";
import { createDiscountSchema } from "../../../services/apiServices/revenue/discountSchema/discountSchemaService";
import AddButton from "../../reusableDesign/AddButton";

const CreateDiscountSchema = ({ clickedIdData }) => {
  const {
    register,
    handleSubmit,
    formState: { errors, isSubmitting },
    setValue,
  } = useForm({ resolver: discountSchemaValidationResolver });
  ...
  useEffect(() => {
    ...
    setValue("discountPercent", clickedIdData?.discountPercent);
  ...
  const onSubmit = (data) => {
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        try {
          createDiscountSchema(data).then((response) => {
            if (response.status === true) {
              toast.success(response.message, {
                icon: "🚀",
                autoClose: 1000,
              });
              router.push("/revenue/discountschema");
              return;
  ...
  return (
    <>
      <CommonHeaderDesign title={"छुट-योजना राख्नुहोस"} />
        ...
        <div className="grid lg:grid-cols-5  gap-5 px-5 pt-10 border border-black border-dashed shadow-2xl bg-gray-100  ">
          ...
          <div className="relative z-0 w-full mb-6 group">
            <input
              ...
              placeholder="."
            />
            <label className="label">छुट दर ( % )</label>

            <p> {errors?.discountPercent?.message}</p>
          </div>
          <div className="relative z-0 w-full mb-6 group">
            <input
              type="number"
...
export default CreateDiscountSchema;
