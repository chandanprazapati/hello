...
import { indexCaseMelMilap } from "../../services/apiServices/legalCase/legalCaseService";
...
export default function ChaluCase() {
  ...
  useEffect(() => {
    const fetchedData = async () => {
      indexCaseMelMilap().then(({ status, data, message }) => {
        try {
          if (status) {
            setApiData(data);
            setLoading(false);
  ...
  return (
    <ListViewPageDesign>
      <SeoOptimization title={"Mel Milap"} />
