...
import { indexCaseStatus } from "../../services/apiServices/legalCase/legalCaseService";
...
export default function ChaluCase() {
  ...
  useEffect(() => {
    const fetchedData = async () => {
      indexCaseStatus(3).then(({ status, data, message }) => {
        try {
          if (status) {
            setApiData(data);
            setLoading(false);
  ...
  return (
    <ListViewPageDesign>
      <SeoOptimization title={"Complete Case"} />
