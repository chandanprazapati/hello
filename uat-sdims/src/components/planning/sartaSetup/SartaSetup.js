...
import {
  deleteSartaSetup,
  sartaSetup,
} from "../../../services/apiServices/planning/sartaSetup/sartaSetupService";
const SartaSetup = () => {
  ...
  const handleEdit = (id) => {
    router.push(`/planning/sarta/createsarta/${id}`);
  };
  ...
  const handleOk = () => {
    deleteSartaSetup(deleteId).then((response) => {
      try {
        response.status === true;
        {
  ...
  return (
    <div className=" rounded-2xl shadow-2xl pb-4 pt-2 px-5 flex flex-col gap-1  divide-black  ">
      ...
      <div className="flex justify-end ">
        <div
          onClick={() => {
            router.push("/planning/sarta/createsarta");
          }}
