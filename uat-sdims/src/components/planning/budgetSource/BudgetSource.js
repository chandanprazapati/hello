...
import {
  budgetSource,
  deleteBudgetSource,
} from "../../../services/apiServices/planning/budgetSource/budgetSourceService";
const BudgetSource = () => {
  ...
  const handleEdit = (id) => {
    router.push(`/planning/budgetsource/createbudgetsource/${id}`);
  };
  ...
  const handleOk = () => {
    deleteBudgetSource(deleteId).then((response) => {
      try {
        response.status === true;
        {
  ...
  return (
    <div className=" rounded-2xl shadow-2xl pb-4 pt-2 px-5 flex flex-col gap-1  divide-black  ">
      ...
      <div className="flex justify-end ">
        <div
          onClick={() => {
            router.push("/planning/budgetsource/createbudgetsource");
          }}
              ...
              {apiData.map((row, index) => {
                return (
                  <TableRow key={index} className="hover:bg-[#a0cae7fd]">
                    ...
                    <TableCell>{row.budgetSourceName}</TableCell>
                    ...
                    <TableCell className="pl-7 cursor-pointer ">
                      <div className="flex gap-10 ">
                        ...
                        <div
                          ...
                          onClick={() => {
                            handleDelete(row.budgetSourceId);
                          }}
                        >
                          <FaTrashAlt size={20} />
                          <DeleteModal
