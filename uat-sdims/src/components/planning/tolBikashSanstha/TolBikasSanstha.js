...
import {
  ...
  tolBikashSanstha,
} from "../../../services/apiServices/planning/tolBikashSanstha/tolBikashSansthaService";
const TolBikasSanstha = () => {
  const router = useRouter();
  const [apiData, setApiData] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let tolBikashSansthaApiData = () => {
    ...
    tolBikashSansthaApiData();
  }, [setApiData]);

  const handleEdit = (id) => {
    router.push(`/planning/tolbikashsanstha/createtolbikashsanstha/${id}`);
  };
  ...
  const handleOk = () => {
    deleteTolBikashSanstha(deleteId).then((response) => {
      try {
        response.status === true;
        {
  ...
  return (
    <div className=" rounded-2xl shadow-2xl pb-4 pt-2 px-5 flex flex-col gap-1  divide-black  ">
      <div className="text-4xl font-bold items-center flex justify-center pt-5  ">
        टोल विकास संस्था समिति सूची
      </div>

      <div className="flex justify-end ">
        <div
          onClick={() => {
            router.push("/planning/tolbikashsanstha/createtolbikashsanstha");
          }}
            ...
            <TableHead>
              <TableRow className="bg-[#3e8dc1fd]">
                ...
                <TableCell sx={{ fontSize: "20px" }}>
                  टोल विकास संस्थाको नाम
                </TableCell>
                <TableCell sx={{ fontSize: "20px" }}>
                  टोल विकास संस्थाको मिति
                </TableCell>
              ...
              {apiData.map((row, index) => {
                return (
                  <TableRow key={index} className="hover:bg-[#a0cae7fd]">
                    ...
                    <TableCell>{row.tolBikashSansthaName}</TableCell>
                    <TableCell>{row.tolSamitiEstdDate}</TableCell>
                    ...
                    <TableCell className="pl-7 cursor-pointer ">
                      <div className="flex gap-10 ">
                        <div
                          ...
                          onClick={() => {
                            handleEdit(row.tolBikashSansthaId);
                          }}
                        >
                          <FaEdit size={20} />
                        </div>
                        <div
                          ...
                          onClick={() => {
                            handleDelete(row.tolBikashSansthaId);
                          }}
                        >
                          <FaTrashAlt size={20} />
                          <DeleteModal
...
export default TolBikasSanstha;
