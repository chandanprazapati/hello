...
import {
  ...
  thekkaShortType,
} from "../../../services/apiServices/planning/thekkaShortType/thekkaShortTypeService";
const ThekkaShortType = () => {
  const router = useRouter();
  const [apiData, setApiData] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let thekkaSortTypeApiData = () => {
    ...
    thekkaSortTypeApiData();
  }, [setApiData]);

  const handleEdit = (id) => {
    router.push(`/planning/thekkashort/createthekkashort/${id}`);
  };
  ...
  const handleOk = () => {
    deleteThekkaShortType(deleteId).then((response) => {
      try {
        response.status === true;
        {
  ...
  return (
    <div className=" rounded-2xl shadow-2xl pb-4 pt-2 px-5 flex flex-col gap-1  divide-black  ">
      <div className="text-4xl font-bold items-center flex justify-center pt-5  ">
        ठेक्का स्रोत प्रकार सूची
      </div>

      <div className="flex justify-end ">
        <div
          onClick={() => {
            router.push("/planning/thekkashort/createthekkashort");
          }}
          ...
          <Table sx={{ minWidth: 700 }} aria-label="customized table">
            <TableHead>
              <TableRow className="bg-[#3e8dc1fd]">
                ...
                <TableCell sx={{ fontSize: "20px" }}>ठेक्का प्रकार</TableCell>
              ...
              {apiData.map((row, index) => {
                return (
                  <TableRow key={index} className="hover:bg-[#a0cae7fd]">
                    ...
                    <TableCell>{row.thekkaShrotName_Nep}</TableCell>
                    ...
                    <TableCell className="pl-7 cursor-pointer ">
                      <div className="flex gap-10 ">
                        <div
                          ...
                          onClick={() => {
                            handleEdit(row.thekkaShrotTypeId);
                          }}
                        >
                          <FaEdit size={20} />
                        </div>
                        <div
                          ...
                          onClick={() => {
                            handleDelete(row.thekkaShrotTypeId);
                          }}
                        >
                          <FaTrashAlt size={20} />
                          <DeleteModal
...
export default ThekkaShortType;
