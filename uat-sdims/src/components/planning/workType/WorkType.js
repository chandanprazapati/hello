...
import {
  deleteWorkType,
  workType,
} from "../../../services/apiServices/planning/workType/workTypeService";
...
const WorkType = () => {
  ...
  const handleEdit = (id) => {
    router.push(`/planning/worktype/createworktype/${id}`);
  };
  ...
  const handleOk = () => {
    deleteWorkType(deleteId).then((response) => {
      try {
        response.status === true;
        {
  ...
  return (
    <div className=" rounded-2xl shadow-2xl pb-4 pt-2 px-5 flex flex-col gap-1  divide-black  ">
      ...
      <div className="flex justify-end ">
        <div
          onClick={() => {
            router.push("/planning/worktype/createworktype");
          }}
