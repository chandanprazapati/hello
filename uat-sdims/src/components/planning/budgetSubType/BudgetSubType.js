...
import {
  budgetSubType,
  deleteBudgetSubType,
} from "../../../services/apiServices/planning/budgetSubType/budgetSubTypeService";
const BudgetSubType = () => {
  ...
  const handleEdit = (id) => {
    router.push(`/planning/budgetsubtype/createbudgetsubtype/${id}`);
  };
  ...
  const handleOk = () => {
    deleteBudgetSubType(deleteId).then((response) => {
      try {
        response.status === true;
        {
  ...
  return (
      ...
      <div className="flex justify-end ">
        <div
          onClick={() => {
            router.push("/planning/budgetsubtype/createbudgetsubtype");
          }}
        ...
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 700 }} aria-label="customized table">
            <TableHead>
                ...
                <TableCell sx={{ fontSize: "20px" }}> बजेटको नाम</TableCell>
            ...
            <TableBody>
              {apiData.map((row, index) => {
                ...
                >
                    ...
                    <TableCell>{row.budgetTypeId}</TableCell>
