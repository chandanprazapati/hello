...
import {
  deleteUpaChetra,
  upaChetra,
} from "../../../services/apiServices/planning/upaChetra/upaChetraService";
...
const UpaChetra = () => {
  ...
  const handleEdit = (id) => {
    router.push(`/planning/upachetra/createupachetra/${id}`);
  };
  ...
  const handleOk = () => {
    deleteUpaChetra(deleteId).then((response) => {
      try {
        response.status === true;
        {
  ...
  return (
    <div className=" rounded-2xl shadow-2xl pb-4 pt-2 px-5 flex flex-col gap-1  divide-black  ">
      ...
      <div className="flex justify-end ">
        <div
          onClick={() => {
            router.push("/planning/upachetra/createupachetra");
          }}
          ...
          <Table sx={{ minWidth: 700 }} aria-label="customized table">
            <TableHead>
              <TableRow className="bg-[#3e8dc1fd]">
                ...
                <TableCell sx={{ fontSize: "20px" }}>उप क्षेत्र</TableCell>
                <TableCell sx={{ fontSize: "20px" }}>खर्च शीर्षक</TableCell>
              ...
              {apiData.map((row, index) => {
                return (
                  <TableRow key={index} className="hover:bg-[#a0cae7fd]">
                    ...
                    <TableCell>{row.upaChettra}</TableCell>
                    <TableCell>{row.kharchaSirshark}</TableCell>
