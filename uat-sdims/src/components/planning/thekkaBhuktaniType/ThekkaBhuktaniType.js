...
import { deleteThekkaBhuktaniType, thekkaBhuktaniType } from "../../../services/apiServices/planning/thekkaBhuktaniType/thekkaBhuktaniTypeService";
const ThekkaBhuktaniType = () => {
  const router = useRouter();
  const [apiData, setApiData] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let thekkaBhuktaniApiData = () => {
    ...
    thekkaBhuktaniApiData();
  }, [setApiData]);

   

  const handleEdit = (id) => {
    router.push(`/planning/thekkabhuktani/createthekkabhuktani/${id}`);
  ...
  const handleOk = () => {
    deleteThekkaBhuktaniType(deleteId).then((response) => {
      try {
        response.status === true;
        {
  ...
  return (
      ...
      ठेक्का भुक्तानी प्रकार सूची
      </div>

      <div className="flex justify-end ">
        <div
          onClick={() => {
            router.push("/planning/thekkabhuktani/createthekkabhuktani");
          }}
        ...
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 700 }} aria-label="customized table">
            <TableHead>
                ...
                <TableCell sx={{ fontSize: "20px" }}> ठेक्का भुक्तानी नाम</TableCell>
            ...
            <TableBody>
              {apiData.map((row, index) => {
                ...
                >
                    ...
                    <TableCell>{row.thekkaBhuktaniTypeName}</TableCell>
                    ...
                    <TableCell className="pl-7 cursor-pointer ">
                      <div className="flex gap-10 ">
                        <div
                          ...
                          onClick={() => {
                            handleEdit(row.thekkaBhuktaniTypeId);
                          }}
                        >
                          <FaEdit size={20} />
                        </div>
                        <div
                          ...
                          onClick={() => {
                            handleDelete(row.thekkaBhuktaniTypeId);
                          }}
                        >
                          <FaTrashAlt size={20} />
                          <DeleteModal
...
export default ThekkaBhuktaniType
