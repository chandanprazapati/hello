...
import {
  deleteWorkArea,
  workArea,
} from "../../../services/apiServices/planning/workArea/workAreaService";
...
const WorkArea = () => {
  ...
  const handleEdit = (id) => {
    router.push(`/planning/workarea/createworkarea/${id}`);
  };
  ...
  const handleOk = () => {
    deleteWorkArea(deleteId).then((response) => {
      try {
        response.status === true;
        {
  ...
  return (
    <div className=" rounded-2xl shadow-2xl pb-4 pt-2 px-5 flex flex-col gap-1  divide-black  ">
      ...
      <div className="flex justify-end ">
        <div
          onClick={() => {
            router.push("/planning/workarea/createworkarea");
          }}
              ...
              {apiData.map((row, index) => {
                return (
                  <TableRow key={index} className="hover:bg-[#a0cae7fd]">
                    ...
                    <TableCell>{row.workAreaName}</TableCell>
