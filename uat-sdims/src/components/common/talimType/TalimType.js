...
export default function TalimType() {
  ...
  useEffect(() => {
    const fetchedData = () => {
      talimType().then(({ status, data, message }) => {
        try {
          if (status) {
            setApiData(data);
            setLoading(false);
  ...
  const handleEdit = useCallback(
    (id) => {
      router.push(`/common/talimType/createTalimType/${id}`);
    },
    [router]
  );
  ...
  return (
    <ListViewPageDesign>
      <ListHeader title="तालिम प्रकार सूची" />
      <ListButton url={`/common/talimType/createTalimType`} />
