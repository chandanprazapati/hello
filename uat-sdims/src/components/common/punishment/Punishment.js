...
export default function Punishment() {
  ...
  useEffect(() => {
    const fetchedData = () => {
      punishment().then(({ status, data, message }) => {
        try {
          if (status) {
            setApiData(data);
            setLoading(false);
  ...
  const handleEdit = useCallback(
    (id) => {
      router.push(`/common/punishment/createPunishment/${id}`);
    },
    [router]
  );
  ...
  return (
    <ListViewPageDesign>
      <ListHeader title="सजाय सूची" />
      <ListButton url={`/common/punishment/createPunishment`} />
