...
export default function Occupation() {
  ...
  useEffect(() => {
    const fetchedData = () => {
      occupation().then(({ status, data, message }) => {
        try {
          if (status) {
            setApiData(data);
            setLoading(false);
  ...
  const handleEdit = useCallback(
    (id) => {
      router.push(`/common/occupation/createOccupation/${id}`);
    },
    [router]
  );
  ...
  return (
    <ListViewPageDesign>
      <ListHeader title="पेशा सूची" />
      <ListButton url={`/common/occupation/createOccupation`} />
