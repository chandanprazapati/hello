...
export default function Counter() {
  ...
  useEffect(() => {
    const fetchedData = () => {
      counter().then(({ status, data, message }) => {
        try {
          if (status) {
            setApiData(data);
            setLoading(false);
  ...
  const handleEdit = useCallback(
    (id) => {
      router.push(`/common/counter/createCounter/${id}`);
    },
    [router]
  );
        ...
        {apiData.map((row, index) => {
          return (
            <TableRow key={index} className="hover:bg-[#a0cae7fd]">
              ...
              <TableCell>{englishToNepali(row.wardId)}</TableCell>
              <TableCell
  ...
  return (
    <ListViewPageDesign>
      <ListHeader title="काउन्टर सूची" />
      <ListButton url="/common/counter/createCounter" />
