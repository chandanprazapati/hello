...
export default function AwardType() {
  ...
  useEffect(() => {
    const fetchedData = () => {
      awardType().then(({ status, data, message }) => {
        try {
          if (status) {
            setApiData(data);
            setLoading(false);
  ...
  const handleEdit = useCallback(
    (id) => {
      router.push(`/common/awardType/createAwardType/${id}`);
    },
    [router]
  );
  ...
  return (
    <ListViewPageDesign>
      <ListHeader title="पुरस्कार सूची" />
      <ListButton url={`/common/awardType/createAwardType`} />
