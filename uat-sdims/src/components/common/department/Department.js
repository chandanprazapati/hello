...
export default function Department() {
  ...
  useEffect(() => {
    const fetchedData = () => {
      department().then(({ status, data, message }) => {
        try {
          if (status) {
            setApiData(data);
            setLoading(false);
  ...
  const handleEdit = useCallback(
    (id) => {
      router.push(`/common/department/createDepartment/${id}`);
    },
    [router]
  );
  ...
  return (
    <ListViewPageDesign>
      <ListHeader title="साखाको प्रकार सूची" />
      <ListButton url={`/common/department/createDepartment`} />
