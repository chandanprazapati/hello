...
export default function OldVdc() {
  ...
  useEffect(() => {
    const fetchedData = () => {
      oldVdc().then(({ status, data, message }) => {
        try {
          if (status) {
            setApiData(data);
            setLoading(false);
  ...
  const handleEdit = useCallback(
    (id) => {
      router.push(`/common/oldVdc/createOldVdc/${id}`);
    },
    [router]
  );
  ...
  return (
    <ListViewPageDesign>
      <ListHeader title="पुरानो गा.वि.स  सूची" />
      <ListButton url={`/common/oldVdc/createOldVdc`} />
