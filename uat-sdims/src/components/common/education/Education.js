...
export default function Education() {
  ...
  useEffect(() => {
    const fetchedData = () => {
      education().then(({ status, data, message }) => {
        try {
          if (status) {
            setApiData(data);
            setLoading(false);
  ...
  const handleEdit = useCallback(
    (id) => {
      router.push(`/common/education/createEducation/${id}`);
    },
    [router]
  );
  ...
  return (
    <ListViewPageDesign>
      <ListHeader title="शिक्षा सूची" />
      <ListButton url={`/common/education/createEducation`} />
