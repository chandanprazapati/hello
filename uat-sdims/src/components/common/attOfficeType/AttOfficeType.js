...
export default function AttOfficeType() {
  ...
  useEffect(() => {
    const fetchedData = () => {
      attOfficeType().then(({ status, data, message }) => {
        try {
          if (status) {
            setApiData(data);
            setLoading(false);
  ...
  const handleEdit = useCallback(
    (id) => {
      router.push(`/common/attOfficeType/createAttOfficeType/${id}`);
    },
    [router]
  );
  ...
  return (
    <ListViewPageDesign>
      <ListHeader title="कार्यालयको हाजिरी प्रकार सूची" />
      <ListButton url={`/common/attOfficeType/createAttOfficeType`} />
