...
export default function Ward() {
  ...
  const handleEdit = useCallback(
    (id) => {
      router.push(`/common/ward/createWard/${id}`);
    },
    [router]
  );
        ...
        {apiData.map((row, index) => {
          return (
            <TableRow key={index} className="hover:bg-[#a0cae7fd]">
              ...
              <TableCell>{englishToNepali(row.palikaId)}</TableCell>
              <TableCell
  ...
  return (
    <ListViewPageDesign>
      <ListHeader title=" वार्ड सूची " />
      <ListButton url="/common/ward/createWard" />
