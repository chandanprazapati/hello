...
export default function Post() {
  ...
  const handleEdit = useCallback(
    (id) => {
      router.push(`/common/post/createPost/${id}`);
    },
    [router]
  );
        ...
        {apiData.map((row, index) => {
          return (
            <TableRow key={index} className="hover:bg-[#a0cae7fd]">
              ...
              <TableCell>{row.shrediId}</TableCell>
  ...
  return (
    <ListViewPageDesign>
      ...
      <ListButton url="/common/post/createPost" />
