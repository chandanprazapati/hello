...
import { overViewHajiri } from "@/services/apiServices/attendance/attendanceService";

const OverViewHajari = () => {
  ...
  const onSubmit = async (formData) => {
    try {
      setLoading(true);
      await overViewHajiri(formData).then((res) => {
        setLoading(false);
        if (res.status === true) {
                ...
                <TableHead>
                  <TableRow className="bg-[#3e8dc1fd]">
                    ...
                    <TableCell sx={{ fontSize: "16px" }}>
                      जम्मा काम गर्नु पर्ने दिन
                    </TableCell>
                    <TableCell sx={{ fontSize: "16px" }}>उपस्थित दिन</TableCell>
                    <TableCell sx={{ fontSize: "16px" }}>
                      अनुपस्थित दिन
                    </TableCell>
                    <TableCell sx={{ fontSize: "16px" }}>
                    ...
                    <TableCell sx={{ fontSize: "16px" }}>
                      विहान हाजिर गर्न बिर्सेको
                    </TableCell>
                    <TableCell sx={{ fontSize: "16px" }}>
                      बेलुकी हाजिर गर्न बिर्सेको
                    </TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {featchData?.map((item, index) => {
                    return (
                      <TableRow key={index} className="hover:bg-[#a0cae7fd]">
                        ...
                        <TableCell>
                          {englishToNepali(item.workingDays)}
                        </TableCell>
                        <TableCell>
                          {englishToNepali(item.presentDays)}
                        </TableCell>
                        <TableCell>
                          {englishToNepali(item.absentDays)}
                        </TableCell>
                        <TableCell>{englishToNepali(item.kajDays)}</TableCell>
                        <TableCell>{englishToNepali(item.leaveDays)}</TableCell>
                        <TableCell>
                          {englishToNepali(item.dhilloAayekoDays)}
                        </TableCell>
                        <TableCell>
                          {englishToNepali(item.chittoGayakoDays)}
                        </TableCell>
                        <TableCell>
                          {englishToNepali(item.forgetMorningAttendanceDays)}
                        </TableCell>
                        <TableCell>
                          {englishToNepali(item.forgetEveningAttendanceDays)}
                        </TableCell>
                      </TableRow>
                    );
                  })}
