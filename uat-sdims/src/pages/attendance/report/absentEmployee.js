...
import { absentEmployee } from "@/services/apiServices/attendance/attendanceService";
...
const AbsentEmployee = () => {
  ...
  const onSubmit = async (formData) => {
    try {
      setLoading(true);
      await absentEmployee(formData).then((res) => {
        setLoading(false);
        if (res.status === true) {
              ...
              <Table sx={{ minWidth: 700 }} aria-label="customized table">
                <TableHead>
                  <TableRow className="bg-[#3e8dc1fd]">
                    ...
                    <TableCell sx={{ fontSize: "16px" }}>मिति</TableCell>
                    <TableCell sx={{ fontSize: "16px" }}>कैफियत</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {featchData?.map((item, index) => {
                    return (
                      <TableRow key={index} className="hover:bg-[#a0cae7fd]">
                        ...
                        <TableCell>
                          {englishToNepali(BS.ADToBS(item.date))}
                        </TableCell>
                        <TableCell>अनुपस्थित</TableCell>
                      </TableRow>
                    );
                  })}
                </TableBody>
