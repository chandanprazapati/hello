...
import { arrivalEarlyDepartureLate } from "@/services/apiServices/attendance/attendanceService";
...
const DhiloBibran = () => {
  ...
  const onSubmit = async (formData) => {
    formData.isLate = formData.isLate === "true";
    setIsLate(formData.isLate);
    try {
      setLoading(true);
      let response = await arrivalEarlyDepartureLate(formData).then((res) => {
        setLoading(false);
        if (res.status === true) {
    ...
    <>
      ...
      <div className=" rounded-2xl shadow-2xl pb-4 pt-2 px-5 flex flex-col gap-1  divide-black   ">
        <div className="text-4xl font-bold items-center flex justify-center pt-5 ">
          ढिला आउने/चाडो जानेको विवरण
        </div>
        <div className="flex container pt-5">
          <form onSubmit={handleSubmit(onSubmit)} className="w-[95%]">
            <div className=" w-full grid lg:grid-cols-5 bg-transparent gap-3 px-5 pt-5 bg-gray-100 border border-black border-dashed">
              ...
              />
              <RadioGroup defaultValue={true} {...register("isLate")}>
                <FormControlLabel
                  value={true}
                  control={<Radio />}
                  label="ढिलो आउने"
              ...
              <Table sx={{ minWidth: 700 }} aria-label="customized table">
                <TableHead>
                  <TableRow className="bg-[#3e8dc1fd]">
                    ...
                    {isLate ? (
                      <TableCell sx={{ fontSize: "16px" }}>आएको समय</TableCell>
                    ) : (
                      <TableCell sx={{ fontSize: "16px" }}>गएको समय</TableCell>
                    )}
                  </TableRow>
                </TableHead>
                <TableBody>
