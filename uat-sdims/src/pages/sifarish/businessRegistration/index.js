...
import { getBusinessRegistration } from "../../../services/apiServices/sifarish/businessRegistration/businessRegistrationService";
import { toast } from "react-toastify";

export default function Index() {
  const router = useRouter();
  const [apiData, setApiData] = useState([]);
  console.log(apiData, "bussRegData");
  ...
  const handleEdit = (id) => {
    router.push(
      `/sifarish/businessRegistration/createBusinessRegistration/${id}`
    );
  };
  const handleDetail = (id) => {
    router.push(`/sifarish/businessRegistration/details/${id}`);
  };
  const handleUploadDocs = (id) => {
    router.push(`/sifarish/businessRegistration/uploadDocs/${id}`);
  };
  const handlePrint = (id) => {
    router.push(
      `/sifarish/businessRegistration/printBusinessRegistration/${id}`
    );
  };
  ...
  return (
    <>
      ...
      <ListViewPageDesign>
        <ListHeader title="व्यवसाय दर्ता सिफारिस सूची" />
        <ListButton
          url={`/sifarish/businessRegistration/createBusinessRegistration`}
        />
