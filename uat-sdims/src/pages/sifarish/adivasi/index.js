...
import SifarishVerifyModal from "../../../components/reusableDesign/SifarishVerifyModal";
...
export default function Index() {
  ...
  const handleEdit = (id) => {
    router.push(`/sifarish/adivasi/createAdivasi/${id}`);
  };

  const handleDetail = (id) => {
    router.push(`/sifarish/adivasi/details/${id}`);
  };

  const handleUploadDocs = (id) => {
    router.push(`/sifarish/adivasi/uploadDocs/${id}`);
  };

  const handlePrint = (id) => {
    router.push(`/sifarish/adivasi/printAdivasi/${id}`);
  };
      ...
      <Table sx={{ minWidth: 700 }} aria-label="customized table">
        <TableHead>
          <TableRow className="bg-[#3e8dc1fd]">
            ...
            <TableCell sx={{ fontSize: "16px" }}>जाती</TableCell>
  ...
  return (
    <React.Fragment>
      <SeoOptimization title={"Adivasi"} />
      <ListViewPageDesign>
        <ListHeader title="आदिवासी/जनजाती सूची" />
        <ListButton url={`/sifarish/adivasi/createAdivasi`} />
