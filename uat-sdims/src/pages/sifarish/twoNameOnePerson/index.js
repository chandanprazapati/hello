...
import { getTwonameOnePerson } from "../../../services/apiServices/sifarish/twoNameOnePerson/twoNameOnePersonService";
...
export default function Index() {
  ...
  useEffect(() => {
    const fetchedData = () => {
      getTwonameOnePerson().then(({ status, data, message }) => {
        try {
          if (status) {
            setApiData(data);
            setLoading(false);
  ...
  const handleEdit = (id) => {
    router.push(`/sifarish/twoNameOnePerson/createTwoNameOnePerson/${id}`);
  };
  const handleDetail = (id) => {
    router.push(`/sifarish/twoNameOnePerson/details/${id}`);
  };
  const handleUploadDocs = (id) => {
    router.push(`/sifarish/twoNameOnePerson/uploadDocs/${id}`);
  };
  const handlePrint = (id) => {
    router.push(`/sifarish/twoNameOnePerson/printTwoNameOnePerson/${id}`);
  };
      ...
      <Table sx={{ minWidth: 700 }} aria-aria-label="customized table">
        <TableHead>
          <TableRow className="bg-[#3e8dc1fd]">
            ...
            <TableCell sx={{ fontSize: "16px" }}>पुरा नाम </TableCell>
            <TableCell sx={{ fontSize: "15px" }}>जन्म मिति</TableCell>
        ...
        <TableBody>
          {apiData.map((row, index) => (
            <TableRow key={index} className="hover:bg-[#a0cae7fd]">
              ...
              <TableCell>{row.actualName_Nepali}</TableCell>
              <TableCell>{row.dateOfBirth_Nepali}</TableCell>
              <TableCell>{row.nagariktaPraPaNo}</TableCell>
  ...
  return (
    <React.Fragment>
      ...
      <ListViewPageDesign>
        <ListHeader title="दुई नाम एक व्यक्ति सूची" />
        <ListButton
          url={`/sifarish/twoNameOnePerson/createTwoNameOnePerson/`}
        />
