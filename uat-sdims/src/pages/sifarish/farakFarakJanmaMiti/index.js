...
import { getFaraJanmakMiti } from "../../../services/apiServices/sifarish/farakFarakJanmaMiti/farakFarakJanmaMitiService";
...
export default function Index() {
  ...
  useEffect(() => {
    const fetchedData = () => {
      getFaraJanmakMiti().then(({ status, data, message }) => {
        try {
          if (status) {
            setApiData(data);
            setLoading(false);
  ...
  const handleEdit = (id) => {
    router.push(
      `/sifarish/farakFarakJanmaMiti/createFarakFarakJanmaMiti/${id}`
    );
  };
  const handleDetail = (id) => {
    router.push(`/sifarish/farakFarakJanmaMiti/details/${id}`);
  };
  const handleUploadDocs = (id) => {
    router.push(`/sifarish/farakFarakJanmaMiti/uploadDocs/${id}`);
  };
  const handlePrint = (id) => {
    router.push(`/sifarish/farakFarakJanmaMiti/printFarakJanmaMiti/${id}`);
  };
      ...
      <Table sx={{ minWidth: 700 }} aria-aria-label="customized table">
        <TableHead>
          <TableRow className="bg-[#3e8dc1fd]">
            ...
            <TableCell sx={{ fontSize: "16px" }}>बाबुको नाम थर</TableCell>
            <TableCell sx={{ fontSize: "15px" }}>आमाको नाम थर</TableCell>
  ...
  return (
    <React.Fragment>
      ...
      <ListViewPageDesign>
        <ListHeader title="फरक फरक जन्म मिति सिफारिस सूची" />
        <ListButton
          url={`/sifarish/farakFarakJanmaMiti/createFarakFarakJanmaMiti`}
        />
