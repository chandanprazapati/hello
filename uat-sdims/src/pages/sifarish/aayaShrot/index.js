...
import { getAayaShrot } from "../../../services/apiServices/sifarish/aayaShrot/aayaShrotService";
...
export default function Index() {
  ...
  const handleEdit = (id) => {
    router.push(`/sifarish/aayaShrot/createAayaShrot/${id}`);
  };
  const handleDetail = (id) => {
    router.push(`/sifarish/aayaShrot/details/${id}`);
  };
  const handleUploadDocs = (id) => {
    router.push(`/sifarish/aayaShrot/uploadDocs/${id}`);
  };
  const handlePrint = (id) => {
    router.push(`/sifarish/aayaShrot/printAayaShrot/${id}`);
  };
      ...
      <Table sx={{ minWidth: 700 }} aria-aria-label="customized table">
        <TableHead>
          <TableRow className="bg-[#3e8dc1fd]">
            ...
            <TableCell sx={{ fontSize: "15px" }}>जम्मा</TableCell>
  ...
  return (
    <React.Fragment>
      <SeoOptimization title={"Aayashrot"} />
      <ListViewPageDesign>
        <ListHeader title="आयश्रोत सिफारिस सूची" />
        <ListButton url={`/sifarish/aayaShrot/createAayaShrot`} />
