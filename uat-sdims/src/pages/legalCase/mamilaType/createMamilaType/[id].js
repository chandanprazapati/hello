...
const CreateMamilaType = dynamic(() => import("./"), {
  ssr: false,
});

export default function CreateMamilaTypeId() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { status, data, message } = await indexCaseSubType();
        if (status === true) {
          setApiData(data);
          setLoading(false);
        } else {
  ...
  return (
    <React.Fragment>
      ...
      {loading ? (
        ...
        <CreateMamilaType clickedIdData={filteredData} />
      )}
    </React.Fragment>
  );
}
