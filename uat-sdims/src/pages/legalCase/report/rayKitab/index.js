...
import { getRayKitabReport} from "../../../../services/apiServices/legalCase/legalCaseService";
...
export default function NivedanPatraDhacha() {
  ...
  useEffect(() => {
    const fetchedData = async () => {
        getRayKitabReport().then(({ status, data, message }) => {
        try {
          if (status) {
            setApiData(data);
            setLoading(false);
        ...
        {apiData.map((row, index) => {
          return (
            <TableRow key={index} className="hover:bg-[#a0cae7fd]">
              ...
              <TableCell> {englishToNepali(row.regDate)}</TableCell>
              ...
              <TableCell> {row.caseStage}</TableCell>
              <TableCell> {row.judgeFinalDecisionDate? englishToNepali(row.judgeFinalDecisionDate) : ""} </TableCell>
              <TableCell> {row.judgeDecisionFinal}</TableCell>
            </TableRow>
          );
        })}
      </TableBody>
  ...
  return (
    <>
    <SeoOptimization title="राय किताब" description="राय किताब" />
          ...
          <div className="bg-[url('https://res.cloudinary.com/dg1opyudu/image/upload/v1684819916/1_ga21iq.jpg')] rounded-xl p-5 w-full ">
            <div className="flex justify-between pb-10  ">
              ...
              <div>
                ...
                <div className="text-center font-bold ">राय किताब</div>
              </div>
...
const tableHeadData = [
  ...
  { id: "mamilaName", name: "	पक्ष निवेदक			" },
  { id: "mamilaType", name: "	विपक्ष		" },
  { id: "mamilaType", name: "	विवाद			" },
  { id: "action", name: "फैसलाको किसिम	" },
  { id: "action", name: "फैसला मिति		" },
  { id: "action", name: "फैसलाको बिवरण	" },
];
