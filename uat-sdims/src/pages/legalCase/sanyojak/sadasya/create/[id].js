...
const CreateSadasya = dynamic(() => import("."), {
  ssr: false,
});

export default function CreateSadasyaById() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { status, data, message } = await sanyojakSadasya();
        if (status === true) {
          setApiData(data);
          setLoading(false);
        } else {
  ...
  return (
    <React.Fragment>
      ...
      {loading ? (
        ...
        <CreateSadasya clickedIdData={filteredData} />
      )}
    </React.Fragment>
  );
}
